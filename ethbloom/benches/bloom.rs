...
#[bench]
fn contains_raw(b: &mut Bencher) {
	let bloom = test_bloom();
	let topic = test_topic();
	let address = test_address();
	b.iter(|| {
		assert!(bloom.contains(Input::Raw(&topic)));
		assert!(bloom.contains(Input::Raw(&address)));
	});
}
...
fn does_not_contain_raw(b: &mut Bencher) {
	...
	b.iter(|| {
		assert!(!bloom.contains(Input::Raw(&dummy)));
		assert!(!bloom.contains(Input::Raw(&dummy2)));
	});
}
...
fn contains_hash(b: &mut Bencher) {
	...
	let address = keccak256(&test_address());
	b.iter(|| {
		assert!(bloom.contains(Input::Hash(&topic)));
		assert!(bloom.contains(Input::Hash(&address)));
	});
}
...
fn does_not_contain_hash(b: &mut Bencher) {
	...
	b.iter(|| {
		assert!(!bloom.contains(Input::Hash(&dummy)));
